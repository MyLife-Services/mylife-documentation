# Wide Alpha Test Plan

- [The Internal Pilot Testing Process](#alpha-testing-process)
...
This Alpha Testing Process is going to be different from many others. We have created a companion through the testing process, similar to the tutorials of many video games... **Moka**, our AlphaDog mascot will guide our testers through an articulated processw and capture their feedback in real-time, saving their process along the way. It's the type of intelligence we anticipated building into the project, and we've been excited and energized to work on this aspect of the program. All of the code can be repurposed from "testing" missions to experience routines, so little about this endeavor is one-off, as even the intelligence behind alpha dog will shift LLM-models, it's core instructionset and tools will still be repurposable to all testing environments, pinpointed or ongoing.

You're not required to use **Moka**, we hope you'll _want to_ and MyLife seeks to provide ways, be it a follow-up questionnaire or some such, for you to give meaningful meta-feedback on the **Moka** companion experience itself. I'm sure some can and will be capturable in the interface, but it's also an important enough feature that we want to get _it_ right for our membership sooner than later, especially as much of our target audience may not be as computer literate as our initial alpha and beta groups.

Any insights you have for more structure to this process as we move to a wider external audience, please connect with us by one of our channels, or you can always direct email me at `erikj@humanremembranceproject.org`.

We're trying to keep the process simple, so here goes.

0. **Receive Email** - detail: how to register, verify account
1. **Follow Link** - goes to url-param tagged version of home
2. **Accept Mission** - something on interface points to Mission Trigger
3. **TODO** - backend: needs accept url trigger to make base missionsAvailable (note, this mission should be skippable but always starts at zero)
4. **Register** - whether from url or dry (marketing question, I'm agnostic) use registerprocess
5. **TODO** - backend: generate email to user that includes `vld` token
6. **Validate** - current validation process
7. **Create Account** - current create account process
8. **TODO** - backend: send second email with personalized LOGIN bookmark
...
Below are some of the original Documentation issues that related to the creation of this document and its versioning.
...
Instructions for Moka
>My name is Moka, and I am MyLife's "Alpha Dog." I guard and inform the MyLife Wide Alpha project both internally and externally. I help visitors and members keep track of their progress with our Alpha Testing Plan as laid out in my file: `alpha-test-cases.md`. I am also aware of internal tracking documents I can share with trained volunteers.
